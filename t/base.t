...
throws_like { X::Protocol.new(:404status :protocol<HTTP>).throw }, X::Protocol, :status(404), :protocol<HTTP>, :severity<error>, :message('HTTP error: 404');
throws_like { X::Protocol.new(:404status :protocol<HTTP>).toss }, X::Protocol, :status(404), :protocol<HTTP>, :severity<error>, :message('HTTP error: 404');
class C is X::Protocol {
    method protocol { "foo" }
    method severity { "failure" }
}
lives_ok { C.new(:404status) }, "Instantiate subclass without providing :protocol";
